...
export async function PATCH(
  ...
  try {
    ...
    const backendResponse = await fetch(
      buildApiUrl(`/api/admin/notifications/${encodeURIComponent(notificationId)}/read`, getInternalApiBaseUrl()),
      {
        method: "PATCH",
        cache: "no-store",
        headers: {
    ...
    console.error("[api/admin/notifications/[notificationId]/read] backend proxy failed", error);
