...
export async function GET(request: Request) {
  try {
    ...
    const backendResponse = await fetch(buildApiUrl(`/api/admin/finance/expenses/export${search}`, getInternalApiBaseUrl()), {
      cache: "no-store",
      headers: {
    ...
    console.error("[api/admin/finance/expenses/export] backend proxy failed", error);
    return NextResponse.json({ error: { category: "system", code: "backend_unavailable", message: "Unable to export expenses right now" } }, { status: 502 });
  }
}
